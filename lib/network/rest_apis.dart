...
Future createCustomer(request) async {
  return handleResponse(
      await MightyAPI().postAsync('store/api/v1/auth/registration', request));
}

Future login(request) async {
  return handleResponse(
      await MightyAPI().postAsync('jwt-auth/v1/token', request));
}
...
Future searchProduct(request) async {
  if (!await isGuestUser() && await isLoggedIn()) {
    return handleResponse(await MightyAPI().postAsync(
        'store/api/v1/woocommerce/get-product', request,
        requireToken: true));
  } else {
    return handleResponse(await MightyAPI().postAsync(
        'store/api/v1/woocommerce/get-product', request,
...
Future getProductAttribute() async {
  return handleResponse(await MightyAPI()
      .getAsync('store/api/v1/woocommerce/get-product-attributes'));
}

Future updateCustomer(id, request) async {
  return handleResponse(
      await MightyAPI().postAsync('wc/v3/customers/$id', request));
}

Future forgetPassword(request) async {
  return handleResponse(await MightyAPI()
      .postAsync('store/api/v1/customer/forget-password', request));
}
...
Future getSubCategories(parent, page) async {
  return handleResponse(await MightyAPI()
      .getAsync('wc/v3/products/categories?page=$page&parent=$parent'));
}
...
Future<List<WishListResponse>> getWishList() async {
  Iterable it = (await handleResponse(await MightyAPI()
      .getAsync('store/api/v1/wishlist/get-wishlist/', requireToken: true)));
  return it.map((e) => WishListResponse.fromJson(e)).toList();
}

Future saveProfileImage(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v2/customer/save-profile-image', request,
      requireToken: true));
}

Future changePassword(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/customer/change-password', request,
      requireToken: true));
}
...
Future addWishList(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/wishlist/add-wishlist/', request,
      requireToken: true));
}

Future removeWishList(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/wishlist/delete-wishlist/', request,
      requireToken: true));
}

Future addToCart(request) async {
  return handleResponse(await MightyAPI()
      .postAsync('store/api/v1/cart/add-cart/', request, requireToken: true));
}

Future removeCartItem(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/cart/delete-cart/', request,
      requireToken: true));
}

Future getCartList() async {
  return handleResponse(await MightyAPI()
      .getAsync('store/api/v1/cart/get-cart/', requireToken: true));
}

Future<CartResponse> getCartListApi() async {
  return CartResponse.fromJson(await handleResponse(await MightyAPI()
      .getAsync('store/api/v1/cart/get-cart/', requireToken: true)));
}

Future updateCartItem(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/cart/update-cart/', request,
      requireToken: true));
}
...
Future postReview(request) async {
  return handleResponse(
      await MightyAPI().postAsync('wc/v3/products/reviews', request));
}

Future updateReview(id1, request) async {
  return handleResponse(
      await MightyAPI().postAsync('wc/v3/products/reviews/$id1', request));
}
...
Future deleteReview(id1) async {
  return handleResponse(
      await MightyAPI().deleteAsync('wc/v3/products/reviews/$id1'));
}

Future getOrders() async {
  return handleResponse(await MightyAPI().getAsync(
      'store/api/v1/woocommerce/get-customer-orders',
      requireToken: true));
}

Future getOrdersTracking(orderId) async {
  return handleResponse(
      await MightyAPI().getAsync('wc/v3/orders/$orderId/notes'));
}
...
Future cancelOrder(orderId, request) async {
  return handleResponse(
      await MightyAPI().postAsync('wc/v3/orders/$orderId', request));
}

Future getCheckOutUrl(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/woocommerce/get-checkout-url', request,
      requireToken: true));
}

Future clearCartItems() async {
  return handleResponse(await MightyAPI()
      .getAsync('store/api/v1/cart/clear-cart/', requireToken: true));
}
...
Future getShippingMethod(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/woocommerce/get-shipping-methods', request,
      requireToken: false));
}
...
Future getVendor() async {
  return handleResponse(
      await MightyAPI().getAsync('store/api/v1/woocommerce/get-vendors'));
}
...
Future socialLoginApi(request) async {
  ...
  return handleResponse(await MightyAPI()
      .postAsync('store/api/v1/customer/social_login', request));
}
...
Future getBlogDetail(id) async {
  return handleResponse(await MightyAPI()
      .getAsync('store/api/v1/blog/get-blog-detail?post_id=$id'));
}
...
Future<bool> updateProfile(
  ...
  var multiPartRequest = MultipartRequest(
      'POST',
      Uri.parse(
          '${getStringAsync(APP_URL)}${'store/api/v2/customer/save-profile-image'}'));
...
Future addTransaction(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'wp/v2/wallet/${getIntAsync(USER_ID)}', request,
      requireToken: true));
}

Future<List<ProductResponse>> getPrivateProduct() async {
  Iterable it = await (handleResponse(
      await MightyAPI().getAsync('wc/v3/products?status=private')));
  return it.map((e) => ProductResponse.fromJson(e)).toList();
}

Future getBalance() async {
  String value = await handleResponse(await MightyAPI().getAsync(
...
Future addWallet(request) async {
  return handleResponse(await MightyAPI().postAsync(
      'store/api/v1/wallet/add-to-wallet', request,
      requireToken: true));
}
