              ...
              return GestureDetector(
                onTap: () async {
                  if (getIntAsync(PRODUCT_DETAIL_VARIANT, defaultValue: 1) ==
                      1) {
                    await ProductDetailScreen1(
                            mProId: cartStore.cartList[i].proId)
                        .launch(context,
                            pageRouteAnimation: PageRouteAnimation.Slide);
                  } else if (getIntAsync(PRODUCT_DETAIL_VARIANT,
                          defaultValue: 1) ==
                      2) {
                    await ProductDetailScreen2(
                            mProId: cartStore.cartList[i].proId)
                        .launch(context,
                            pageRouteAnimation: PageRouteAnimation.Slide);
                  } else if (getIntAsync(PRODUCT_DETAIL_VARIANT,
                  ...
                  children: <Widget>[
                    if (cartStore.cartList.isNotEmpty)
                      cartStore.cartList[i].full == null
                          ? CachedNetworkImage(
                                  imageUrl: cartStore.cartList[i].gallery!
                                      .validate()
                                      .toString()[0],
                                  fit: BoxFit.cover,
                                  height: 85,
                          ...
                          : CachedNetworkImage(
                                  imageUrl: cartStore.cartList[i].full
                                      .toString()
                                      .validate(),
                                  fit: BoxFit.cover,
                                  height: 85,
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(cartStore.cartList[i].name,
                              maxLines: 2, style: primaryTextStyle(size: 15)),
                          8.height,
                          Row(
                                ...
                                children: [
                                  PriceWidget(
                                    price: nf.format(double.parse(
                                            cartStore.cartList[i].price) *
                                        double.parse(
                                            cartStore.cartList[i].quantity)),
                                    size: 16,
                                  ),
                                  PriceWidget(
                                    price: cartStore.cartList[i].regularPrice
                                            .toString()
                                            .isEmpty
                                        ? ''
                                        : nf.format(double.parse(cartStore
                                                .cartList[i].regularPrice) *
                                            double.parse(cartStore
                                                .cartList[i].quantity)),
                                    size: 16,
                                    color: textSecondaryColour,
                                    isLineThroughEnabled: true,
                                  ).paddingOnly(left: 2).visible(cartStore
                                          .cartList[i].salePrice
                                          .toString()
                                          .validate()
                                          .isNotEmpty &&
                                      cartStore.cartList[i].onSale == true),
                                ],
                              ).expand(),
                              Row(
                                children: <Widget>[
                                    ...
                                    onTap: () async {
                                      ...
                                      if (qty == 1 || qty < 1) {
                                        ...
                                        if (!await isGuestUser()) {
                                          var request = {
                                            'pro_id':
                                                cartStore.cartList[i].proId,
                                            'cart_id':
                                                cartStore.cartList[i].cartId,
                                            'quantity': qty,
                                          };
                                          ...
                                          mCartModel.name =
                                              cartStore.cartList[i].name;
                                          mCartModel.proId =
                                              cartStore.cartList[i].proId;
                                          mCartModel.onSale =
                                              cartStore.cartList[i].onSale;
                                          mCartModel.salePrice =
                                              cartStore.cartList[i].salePrice;
                                          mCartModel.regularPrice = cartStore
                                              .cartList[i].regularPrice;
                                          mCartModel.price =
                                              cartStore.cartList[i].price;
                                          mCartModel.gallery =
                                              cartStore.cartList[i].gallery;
                                          mCartModel.quantity = qty.toString();
                                          mCartModel.full =
                                              cartStore.cartList[i].full;
                                          mCartModel.cartId =
                                              cartStore.cartList[i].cartId;
                                          cartStore.removeFromCartList(
                                  ...
                                  Text(cartStore.cartList[i].quantity,
                                      style: boldTextStyle()),
                                  GestureDetector(
                                    onTap: () async {
                                      var qty = int.parse(
                                      ...
                                      if (!await isGuestUser()) {
                                        ...
                                        mCartModel.regularPrice =
                                            cartStore.cartList[i].regularPrice;
                                        mCartModel.price =
                                            cartStore.cartList[i].price;
                                        mCartModel.gallery =
                                            cartStore.cartList[i].gallery;
                          ...
                          Text(appLocalization.translate('lbl_sold_out')!,
                              ...
                              .paddingLeft(16)
                              .visible(cartStore.cartList[i].stockStatus ==
                                  "outofstock"),
