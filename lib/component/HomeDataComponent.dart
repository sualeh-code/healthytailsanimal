...
Future fetchCategoryData() async {
  await getCategories(1, TOTAL_CATEGORY_PER_PAGE).then((res) {
    ...
    // List<Category> mymodel;
    // mCategoryModel
    //     .sort((a, b) => b.count.toString().compareTo(b.count.toString()));
    // mCategoryModel
    //     .sort((a, b) => b.count.toString().compareTo(a.count.toString()));

    for (var i = 0; i < mCategoryModel.length; i++) {
      if (i == 0 && mCategoryModel.length != 0) {
        for (var j = 0; j < mCategoryModel.length; j++) {
          if (mCategoryModel[j].id == 88) {
            mymodel.add(mCategoryModel[j]);
          }
        }
      }
      if (i == 1) {
        for (var j = 0; j < mCategoryModel.length; j++) {
          if (mCategoryModel[j].id == 87) {
            mymodel.add(mCategoryModel[j]);
          }
        }
      }
      ...
      if (i > 3) {
        for (var j = 0; j < mCategoryModel.length; j++) {
          if (mCategoryModel[j].id != 88 &&
              mCategoryModel[j].id != 87 &&
              mCategoryModel[j].id != 20 &&
              mCategoryModel[j].id != 89) {
            mymodel.add(mCategoryModel[j]);
          }
        }
      }
    }
    mCategoryModel = mymodel;
  }).catchError((error) {
    log(error);
  });
}
  ...
  await isNetworkAvailable().then((bool) async {
    if (bool) {
      ...
      await getDashboardApi().then((res) async {
        await setValue(DEFAULT_CURRENCY,
  ...
  return Container(
    ...
    child: Column(
      children: [
        ...
        10.height,
        Text("'" + mQuotes[rnd.nextInt(mQuotes.length)]! + "'",
