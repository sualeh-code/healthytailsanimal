...
Future<void> setDynamicStatusBarColor(
  ...
  if (getIntAsync(DASHBOARD_PAGE_VARIANT, defaultValue: 1) == 1) {
    setStatusBarColor(
        color ?? primaryColor! /*, statusBarIconBrightness: Brightness.light*/,
    ...
    setStatusBarColor(
        color ??
            primaryColor! /*, statusBarIconBrightness: appStore.isDarkMode ? Brightness.light : Brightness.dark*/,
        delayInMilliSeconds: milliseconds);
  }
}
...
void addItemToWishlist(
    {ProductResponse? data, ProductDetailResponse? mDetailResponse}) {
...
void checkWishList(ProductDetailResponse? mDetailResponse, context) async {
  if (!await isLoggedIn())
    ...
    print("IsInWish" +
        wishListStore.isItemInWishlist(mDetailResponse!.id!).toString());
    List<String> mList = [];
    mDetailResponse.images.forEachIndexed((element, index) {
      mList.add(element.src!);
    });
...
Future<void> addCart(
    {ProductResponse? datum,
    ProductDetailResponse? data,
    WishListResponse? mData,
  ...
  if (datum != null && data == null) {
    ...
    log("proID$proID");
    List<String?> mList = [];
    datum.images.forEachIndexed((element, index) {
      mList.add(element.src);
    });

    mCartModel.name = datum.name;
    mCartModel.proId = proID.toString().isEmptyOrNull ? datum.id : proID;
    mCartModel.onSale = datum.onSale;
    mCartModel.salePrice = datum.salePrice;
    mCartModel.regularPrice = datum.regularPrice;
    mCartModel.price = datum.price;
    mCartModel.gallery = mList;
    mCartModel.quantity = "1";
    mCartModel.stockQuantity = "1";
    mCartModel.stockStatus = "";
    mCartModel.thumbnail = "";
    mCartModel.full = datum.images![0].src;
    mCartModel.cartId = datum.id;
    mCartModel.sku = "";
    mCartModel.createdAt = "";

    if (cartStore.isItemInCart(datum.id.validate())) {
...
Future<void> addCartHome({ProductResponse? data}) async {
  CartModel mCartModel = CartModel();
  if (data != null) {
    var proID = data.id;
    log("proID$proID");
