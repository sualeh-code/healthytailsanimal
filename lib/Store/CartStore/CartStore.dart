  ...
  void calculateTotal() {
    ...
    for (var i = 0; i < cartList.length; i++) {
      ...
      if (cartList[i].onSale) {
        cartTotalCount += double.parse(cartList[i].salePrice) *
            int.parse(cartList[i].quantity);
        log("cartTotalCount=$cartTotalCount");
        cartTotalAmount += double.parse(cartList[i].regularPrice) *
            int.parse(cartList[i].quantity);
        ...
        cartTotalCount += double.parse(
                cartList[i].regularPrice.toString().isNotEmpty
                    ? cartList[i].regularPrice
                    : cartList[i].price) *
            int.parse(cartList[i].quantity);
        cartTotalAmount += double.parse(
                cartList[i].regularPrice.toString().isNotEmpty
                    ? cartList[i].regularPrice
                    : cartList[i].price) *
            int.parse(cartList[i].quantity);
